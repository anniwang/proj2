...
// An implementation of a dictionary as a non*resizable* hash table with
// chaining.

class FixedChainHashDict : public PredDict
  {
  public:
    FixedChainHashDict(int table_size); // create table of fixed size
    ~FixedChainHashDict();
  ...
  private:
    ...
    // store the UniqId string in keyID. (for insert) 

    // Then, when you do a find, 
    // have a helper function that calls getUniqId() ONCE
    ...
    int *probes_stats; // probe_stats[i] = how often i probes needed
    const static int MAX_STATS = 20; // How big to make the array.

    int hash(string keyID); // The hash function should not be modified
    bool find_helper(string keyID, PuzzleState *&pred); // helper function for search()
  };
